...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="max-w-4xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div class="space-y-2">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900">Edit Salary Record</h1>
                            <p class="text-sm text-gray-500 mt-1">Update salary payment information</p>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
            ...
            <form action="{{ route('staff.salary.update', $salary->id) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="relative">
                            ...
                            <select name="staff_id" id="staff_id"
                                ...
                                @foreach($staff as $member)
                                    <option value="{{ $member->id }}" {{ old('staff_id', $salary->staff_id) == $member->id ? 'selected' : '' }}>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="amount" id="amount" value="{{ old('amount', $salary->amount) }}" step="0.01" min="0"
                                   class="pl-8 w-full border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 py-3"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <select name="month" id="month"
                                ...
                                <option value="January" {{ old('month', $salary->month) == 'January' ? 'selected' : '' }}>January</option>
                                <option value="February" {{ old('month', $salary->month) == 'February' ? 'selected' : '' }}>February</option>
                                <option value="March" {{ old('month', $salary->month) == 'March' ? 'selected' : '' }}>March</option>
                                <option value="April" {{ old('month', $salary->month) == 'April' ? 'selected' : '' }}>April</option>
                                <option value="May" {{ old('month', $salary->month) == 'May' ? 'selected' : '' }}>May</option>
                                <option value="June" {{ old('month', $salary->month) == 'June' ? 'selected' : '' }}>June</option>
                                <option value="July" {{ old('month', $salary->month) == 'July' ? 'selected' : '' }}>July</option>
                                <option value="August" {{ old('month', $salary->month) == 'August' ? 'selected' : '' }}>August</option>
                                <option value="September" {{ old('month', $salary->month) == 'September' ? 'selected' : '' }}>September</option>
                                <option value="October" {{ old('month', $salary->month) == 'October' ? 'selected' : '' }}>October</option>
                                <option value="November" {{ old('month', $salary->month) == 'November' ? 'selected' : '' }}>November</option>
                                <option value="December" {{ old('month', $salary->month) == 'December' ? 'selected' : '' }}>December</option>
                            </select>
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="year" id="year" value="{{ old('year', $salary->year) }}" min="2000" max="2100"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="date" name="payment_date" id="payment_date" value="{{ old('payment_date', $salary->payment_date ? $salary->payment_date->format('Y-m-d') : '') }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="Overdue" {{ old('status', $salary->status) == 'Overdue' ? 'selected' : '' }}>Overdue</option>
                            </select>
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                      class="pl-10 w-full border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 py-3"
                                      placeholder="Additional notes about this salary payment">{{ old('notes', $salary->notes) }}</textarea>
                        </div>
                        @error('notes')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
